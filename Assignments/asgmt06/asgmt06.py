...
class Knapsack():
	...
	def initTable(self):
		"""Initialize the return dictionary and itemCount array"""
		for i in range(self.w + 1):
			self.table[i] = [-1, -1]	# [min calories, number of item]

		self.itemCount = [0] * self.n
	...
	def mCal_Iterative(self):
		...
		# Initialize variables
		min_val = float('INF')		# GOOD
		cur_val = 0					# GOOD
		min_item = -1				# 
		cur_item = -1				# 
		self.table[0][0] = 0		# GOOD
	...
	def runIterative(self):
		...
		if (min_cal < float('INF') and min_cal > 0):
			print("    Possible to spend exactly: {}".format(self.w))
			print("    Minimum calories: {}".format(min_cal))
...
def main():
	...
	# Re-initialize table and array for iterative section
	k.initTable()
